        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.cunzaiwenti" label="存在问题" prop="cunzaiwenti">
                    <span>{{ruleForm.cunzaiwenti}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="项目点评" prop="xiangmudianping">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="项目点评"
                  v-model="ruleForm.xiangmudianping" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.xiangmudianping" label="项目点评" prop="xiangmudianping">
                    <span>{{ruleForm.xiangmudianping}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.cunzaiwenti = true;
          ...
          if(o=='xiangmudianping'){
            this.ruleForm.xiangmudianping = obj[o];
	    this.ro.xiangmudianping = true;
            continue;
          }
          if(o=='fabushijian'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhidaoxiangmu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
